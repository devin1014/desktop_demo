class EmployeeUtil {
  ...
  // 校验身份证合法性
  static String? verifyIdentifyId(String cardId) {
    ...
    if (_city[int.parse(cardId.substring(0, 2))] == null) {
      String tip = '地址编码错误';
      print(tip);
      return tip;
    }
    ...
    if (cardId.length == 18) {
      ...
      if (last.toString() != numList[17]) {
        String tip = "校验位错误";
        print(tip);
        return tip;
      }
    } else {
      String tip = '身份证号不是18位';
      print(tip);
      return tip;
    }
    return null;
...
// 根据身份证号获取年龄
  int getAgeFromCardId(String cardId) {
    bool isRight = verifyIdentifyId(cardId) != null;
    if (!isRight) {
      return 0;
    }
...
// 根据身份证获取性别
  String getSexFromCardId(String cardId) {
    String sex = "";
    bool isRight = verifyIdentifyId(cardId) != null;
    if (!isRight || cardId.length != 18) return sex;
